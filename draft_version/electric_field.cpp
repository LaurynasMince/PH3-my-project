...
void electric_field(float v[], float grid_spacing, int original_grid_rows, int original_grid_cols) {

  // Defining and initializing constants
  const int grid_rows = original_grid_rows - 1;
  const int grid_cols = original_grid_cols - 1;

  // Define 1D arrays simulating 2D arrays to store values of electric field
  float Ex[(grid_rows) * (grid_cols)] = { 0 };
  float Ey[(grid_rows) * (grid_cols)] = { 0 };
  float Ex_prime[(grid_rows) * (grid_cols)] = { 0 };
  float Ey_prime[(grid_rows) * (grid_cols)] = { 0 };
