...
int main(void) {
  ...
  for (int row=0; row < grid_rows; row++) {
	...
	v[row][col] = (1/4.0)*(v[row+1][col] + v[row][col-1] + v[row][col+1]);
  ...
  for (int k=1; k <= max_iterate; k++) {
	...
	// If the point is on the edge, find its value by consider three, instead of four, points as before.
	  ...
	  v[row][col] = (1-s)*v[row][col] + (s/4.0)*(v[row+1][col] + v[row][col-1] + v[row][col+1]);
	else if (row == (grid_rows - 1))
	  v[row][col] = (1-s)*v[row][col] + (s/4.0)*(v[row-1][col] + v[row][col-1] + v[row][col+1]);
	else
  ...
  for (int row=0; row < (grid_rows-1); row++){
    for (int col=0; col < (grid_cols-1); col++){
      ...
      Ex[row][col]= (v[row][col] - v[row][col+1])/grid_spacing;
      Ey[row][col]= (v[row][col] - v[row+1][col])/grid_spacing;
    }
  }
...
for (int row=0; row < (grid_rows-1); row++){
    for (int col=0; col < (grid_cols-1); col++){
      Ex_prime[row][col]= 0.5 *(Ex[row+1][col] + Ex[row][col]);
      Ey_prime[row][col]= 0.5 * (Ey[row][col+1] + Ey[row][col]);
    }
 }
