...
int main(void) {
  ...
  float s = 4 / (2 + sqrt(4 - ((cos(PI/(grid_rows-1)) + cos(PI/(grid_cols-1))) * (cos(PI/(grid_rows-1)) + cos(PI/(grid_cols-1))))));
  int max_iterate = 50000;
  float err_bound = pow(10, -2);
  ...
  for (int k=1; k <= max_iterate; k++) {
	...
	// If the point is on the edge, find its value by considering three, instead of four, points as before.
	else if (row == 0) {
	  new_v[row * grid_cols + col] = (1-s)*v[row * grid_cols + col] + (s/4.0)*(v[(row+1) * grid_cols + col] + v[row * grid_cols + (col-1)] + v[row * grid_cols + (col+1)]);
	  // cout << abs(test - v[row * grid_cols + col]) << endl;
	  // v[row * grid_cols + col] = test;
	}
	else if (row == (grid_rows - 1))
	  new_v[row * grid_cols + col] = (1-s)*v[row * grid_cols + col] + (s/4.0)*(v[(row-1) * grid_cols + col] + v[row * grid_cols + (col-1)] + v[row * grid_cols + (col+1)]);
	else {
    ...
    error_check(v, new_v, grid_rows, grid_cols, err_bound);
    equate_matrix(v, new_v, grid_rows, grid_cols);

    /*
    int a = error_check(v, new_v, grid_rows, grid_cols, err_bound);
    if (a == 0) {
      cout << "Number of iterations used: " << k << endl;
      break;
    }
    else if(a == 1)
      equate_matrix(v, new_v, grid_rows, grid_cols);
